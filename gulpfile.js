...
gulp.task('css-libs', ['sass'], function () {
	return gulp.src([ // Берем все необходимые библиотеки
			...
			// 'app/sass/partials/swiper-bundle.min.css',
		])
...
gulp.task('sass', function () { // Создаем таск Sass
	return gulp.src('app/sass/**/*.scss') // Берем источник
		...
		.pipe(autoprefixer(['last 15 versions', '> 1%', 'ie 8', 'ie 7', 'iOS >= 9'], {
			cascade: true
		})) // Создаем префиксы
		.pipe(sourcemaps.write(''))
...
gulp.task('watch', ['browser-sync', 'css-libs', 'scripts'], function () {
	...
	gulp.watch('app/sass/**/*.scss', function (event, cb) {
		setTimeout(function () {
			gulp.start('sass');
		}, 500) // задача выполниться через 500 миллисекунд и файл успеет сохраниться на диске
	});
